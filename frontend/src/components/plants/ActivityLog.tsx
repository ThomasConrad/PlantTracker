...
export const ActivityLog: Component<ActivityLogProps> = (props) => {
    ...
    try {
      ...
      if (selectedPhotos().length > 0) {
        ...
        for (const photo of selectedPhotos()) {
          try {
            const uploadedPhoto = await plantsStore.uploadPhoto(
              props.plant.id,
              photo,
            );
      ...
      const entryData: CreateTrackingEntryRequest = {
        timestamp: new Date().toISOString(),
        careTaskIds:
          selectedCareTaskIds().length > 0 ? selectedCareTaskIds() : undefined,
  ...
  const getEntryLabel = (entry: TrackingEntry): string => {
    ...
    if (entry.careTaskIds && entry.careTaskIds.length > 0) {
      const taskNames = entry.careTaskIds
        .map(
          (id) => (props.plant.careTasks ?? []).find((t) => t.id === id)?.name,
        )
        .filter(Boolean);
  ...
  const getEntryIcon = (entry: TrackingEntry) => {
    if (entry.careTaskIds && entry.careTaskIds.length > 0) {
      ...
      const task = (props.plant.careTasks ?? []).find(
      ...
      <div class="card-body">
        ...
        <Show when={showAddForm()}>
          <form
            ...
            class="space-y-4 p-4 bg-gray-50 rounded-lg mb-6"
          >
            {/* Care Tasks (multi-select) */}
            <Show when={careTasks().length > 0}>
              <div class="space-y-2">
                <label class="label">
                  Care Tasks (optional, select multiple)
                </label>
              ...
              {(entry) => (
                <div class="flex items-start space-x-3 p-3 bg-white border border-gray-200 rounded-lg">
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <Show
                      when={entry.measurements && entry.measurements.length > 0}
                    >
                          ...
                          {(m) => {
                            ...
                            return (
                              <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-purple-100 text-purple-800">
                                {metric?.name ?? "Metric"}: {String(m.value)}{" "}
                                {metric?.unit ?? ""}
                              </span>
                            );
                          }}
