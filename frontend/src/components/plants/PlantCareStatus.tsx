...
export const PlantCareStatus: Component<PlantCareStatusProps> = (props) => {
  ...
  const scheduledTasks = () => careTasks().filter(t => t.intervalDays != null && !t.archivedAt);
  const unscheduledTasks = () => careTasks().filter(t => t.intervalDays == null && !t.archivedAt);
      ...
      <div class="p-4 sm:p-6 space-y-8">
          ...
          <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <For each={scheduledTasks()}>
                {(task) => (
                  <div class={`p-4 rounded-lg border ${getTaskStatusColor(task)}`}>
                    <div class="flex items-center justify-between">
                      <div class="flex items-center min-w-0">
                        <span class="text-lg mr-2 flex-shrink-0">{task.icon ?? '🌱'}</span>
                        <div class="min-w-0">
                          <h4 class="text-sm font-medium truncate">{task.name}</h4>
                          <p class="text-xs font-semibold">{getTaskStatusText(task)}</p>
        ...
        <Show when={careTasks().filter(t => !t.archivedAt).length === 0}>
          <div class="text-center py-6 bg-gray-50 rounded-lg">
            <p class="text-sm text-gray-600 font-medium">No care tasks configured</p>
            <p class="text-xs text-gray-500 mt-1">Add care tasks to track watering, fertilizing, and more</p>
          </div>
        </Show>
        ...
        <div>
          ...
          <Show when={!loading() && recentEntries().length === 0}>
            <div class="text-center py-6 bg-gray-50 rounded-lg">
              ...
              <p class="text-xs text-gray-500 mt-1">Use the care tasks above to log activities</p>
            </div>
          </Show>
                ...
                {(entry) => (
                  <div class="flex items-center space-x-3 p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors duration-150">
                    ...
                    <div class="flex-1 min-w-0">
                      ...
                      <Show when={entry.photoIds && Array.isArray(entry.photoIds) && (entry.photoIds as string[]).length > 0}>
