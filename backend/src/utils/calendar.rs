...
mod tests {
    ...
    fn create_test_plant_with_name(
        name: &str,
        genus: &str,
        watering_days: i32,
        fertilizing_days: i32,
    ) -> PlantResponse {
        PlantResponse {
            id: Uuid::new_v4(),
            name: name.to_string(),
    ...
    fn test_generate_calendar_with_multiple_plants() {
        ...
        assert!(
            has_fiddle || has_snake || has_pothos,
            "No plants found in calendar"
        );
        ...
        assert!(
            has_intervals,
            "Calendar should include interval information"
        );
    }
    ...
    fn test_generate_calendar_feed_url() {
        ...
        assert_eq!(
            url2,
            "http://example.com:3000/api/v1/calendar/abc-def.ics?token=xyz789"
        );
    }
