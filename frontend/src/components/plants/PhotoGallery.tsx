...
export const PhotoGallery: Component<PhotoGalleryProps> = (props) => {
  ...
  const timelinePhotos = createMemo(() => {
    ...
    return photosByOldest().map((photo) => {
      const daysSincePrevious = lastDate ? getDaysBetween(lastDate, photo.createdAt) : null;
  ...
  const handleDeletePhoto = async (photoId: string, fromModal = false) => {
    try {
      ...
      if (fromModal) {
        const remaining = photosByNewest().filter((photo) => photo.id !== photoId);
  ...
  return (
    <div class="bg-white shadow-sm rounded-xl sm:rounded-2xl border border-gray-200 overflow-hidden">
      <div class="px-4 sm:px-6 py-4 sm:py-5 border-b border-gray-100 bg-gray-50/50">
        <div class="flex flex-wrap items-center justify-between gap-3">
          ...
          <div class="flex items-center gap-2">
            ...
            <Button variant="outline" size="sm" onClick={() => fileInputRef?.click()} loading={uploading()}>
          ...
          <Show
            ...
            fallback={
              <div class="text-center py-10">
                ...
                <p class="mt-1 text-sm text-gray-500">Start documenting your plant's growth.</p>
              </div>
            }
          >
            <Show
                  ...
                  <Show when={view() === 'timeline'}>
                    ...
                    <For each={timelinePhotos()}>
                      {(item, index) => {
                        ...
                        const showMonth = !previous || previous.monthLabel !== item.monthLabel;
                          ...
                          <div class="relative pb-8 last:pb-0">
                            ...
                            <div class="pl-6 sm:pl-8">
                              <button
                                ...
                                onClick={() => openPhotoModalById(item.photo.id)}
                              >
                                <img
                                  src={apiClient.getPhotoUrl(props.plantId, item.photo.id)}
                                ...
                                <div class="p-3 sm:p-4 flex items-center justify-between gap-2">
                                  ...
                                  <p class="text-xs text-gray-500">{formatFullDate(item.photo.createdAt)}</p>
                                </div>
                              </button>
                            </div>
                          </div>
      ...
      <Show when={showPhotoModal() && selectedPhotoIndex() !== null && currentPhoto()}>
        <div class="fixed inset-0 bg-black/90 flex items-center justify-center z-[70]" onClick={closePhotoModal}>
          <div class="relative w-full h-full flex items-center justify-center p-4">
            ...
            <div class="absolute bottom-4 left-1/2 -translate-x-1/2 w-[min(92vw,38rem)] bg-black/55 text-white px-4 py-3 rounded-xl" onClick={(e) => e.stopPropagation()}>
              <div class="flex flex-wrap items-center justify-between gap-2">
                <p class="text-sm font-medium">{formatFullDate(currentPhoto()!.createdAt)}</p>
              ...
              <div class="mt-3 flex items-center gap-2">
                <Button
                  ...
                  onClick={() => void handleSetPreview(currentPhoto()!.id, true)}
                >
                  Set as preview
                </Button>
                <Button
                  ...
                  onClick={() => void handleDeletePhoto(currentPhoto()!.id, true)}
                >
                  Delete
                </Button>
              </div>
