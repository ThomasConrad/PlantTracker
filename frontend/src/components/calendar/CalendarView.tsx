...
export const CalendarView: Component<CalendarViewProps> = (props) => {
  ...
  const weekStartOffset = createMemo(() =>
    authStore.user?.firstDayOfWeek === "monday" ? 1 : 0,
  );
        ...
        try {
          ...
          for (const entry of response.entries) {
            ...
            if (
              ...
              !props.selectedTypes.includes(entryType)
            ) {
              continue;
            }
            allEvents.push({
  ...
  const setupMobileGestures = () => {
    if (!gridContainerRef || !gridTrackRef || !overlayRef || !overlayScrollRef)
      return;
    ...
    const onGridTouchEnd = () => {
      ...
      if (velocity > FLING_THRESHOLD || dx > gridContainerW * SWIPE_FRACTION)
        swipeDir = -1;
      else if (
        velocity < -FLING_THRESHOLD ||
        dx < -gridContainerW * SWIPE_FRACTION
      )
        swipeDir = 1;
    ...
    container.addEventListener("touchstart", onGridTouchStart, {
      passive: true,
    });
    container.addEventListener("touchmove", onGridTouchMove, {
      passive: false,
    });
    ...
    const onOverlayTouchMove = (e: TouchEvent) => {
      ...
      if (
        !overlayDragging &&
        (e.target as HTMLElement)?.closest(".mc-agenda-scroll")
      ) {
    ...
    overlay.addEventListener("touchmove", onOverlayTouchMove, {
      passive: false,
    });
    ...
    if (weekdaysEl) {
      weekdaysEl.addEventListener("touchstart", onGridOverlayTouchStart, {
        passive: true,
      });
      weekdaysEl.addEventListener("touchmove", onOverlayTouchMove, {
        passive: false,
      });
      ...
      requestAnimationFrame(() => {
        if (overlayRef) {
          ...
          const target = sheetOpen()
            ? HEADER_HEIGHT
            : gridContainerRef!.getBoundingClientRect().bottom;
          overlayRef.style.transition = SNAP_TRANSITION;
          const viewportH = window.visualViewport?.height || window.innerHeight;
          overlayRef.style.height =
            Math.max(120, viewportH - HEADER_HEIGHT) + "px";
  ...
  const renderCalendarGrid = (
    days: Date[],
    isCurrentMonthFn: (d: Date) => boolean,
  ) => (
      ...
      <Show when={isMobile()}>
        <div class="mc-mobile-root">
          ...
          <div class="mc-weekdays">
            <For each={dayNamesShort()}>{(day) => <span>{day}</span>}</For>
          </div>
          ...
          <div class="mc-agenda-overlay" ref={overlayRef}>
            <button
              class="mc-agenda-handle"
              onClick={handleOverlayHandleClick}
            ></button>
                ...
                {(day) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        class={
                          isTodayDay ? "calendar-date-today" : "calendar-date"
                        }
                      >
                        {day.getDate()}
                      </div>
