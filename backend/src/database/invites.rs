...
pub async fn list_invite_codes(
    ...
    let invite_rows = if let Some(user_id) = created_by {
        ...
        sqlx::query_as::<_, InviteCodeRow>("SELECT * FROM invite_codes ORDER BY created_at DESC")
            .fetch_all(pool)
            .await
            .map_err(AppError::Database)?
    };
    ...
    .map_err(|e| {
        if e.to_string().contains("unique constraint") {
            AppError::Validation({
                ...
                errors.add("email", validator::ValidationError::new("already_exists"));
                errors
            })
        } else {
            AppError::Database(e)
...
pub async fn get_waitlist_entries(pool: &DatabasePool) -> Result<Vec<WaitlistEntry>> {
    let entry_rows =
        sqlx::query_as::<_, WaitlistEntryRow>("SELECT * FROM waitlist ORDER BY created_at ASC")
            .fetch_all(pool)
            .await
            .map_err(AppError::Database)?;
...
pub async fn update_waitlist_status(
    ...
    let now = Utc::now();
    let now_str = now.to_rfc3339();
    let invited_at_str = if status == "invited" {
        Some(now_str.clone())
    } else {
        None
    };
