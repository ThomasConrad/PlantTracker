...
interface CareTypeButtonProps extends Omit<
...
export const CareTypeButton: Component<CareTypeButtonProps> = (props) => {
  ...
  const mapToCareIconType = (
    type: EntryType,
  ): Parameters<typeof CareIcon>[0]["type"] => {
    switch (type) {
      case "care":
        return "watering";
      case "measurement":
  ...
  return (
    <button
      class={cn(
        ...
        careTypeClasses[local.careType as keyof typeof careTypeClasses] ||
          careTypeClasses.care,
        local.class,
      )}
      {...rest}
    >
          ...
          <>
            {local.showIcon !== false && (
              <CareIcon
                ...
                size={
                  local.size === "lg" ? "lg" : local.size === "sm" ? "sm" : "md"
                }
              />
            )}
