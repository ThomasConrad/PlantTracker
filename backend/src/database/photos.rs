...
pub async fn create_photo(
    ...
    // TODO: Save actual file to storage (filesystem, S3, etc.)
    // For now, we'll just store the metadata

    sqlx::query(
        "INSERT INTO photos (id, plant_id, filename, original_filename, size, content_type, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?)"
    )
...
pub async fn delete_photo(
    ...
    // Get photo info for file cleanup
    let photo_row = sqlx::query(
        "SELECT filename FROM photos WHERE id = ? AND plant_id = ?"
    )
    .bind(photo_id.to_string())
    .bind(plant_id.to_string())
    .fetch_optional(pool)
    ...
    let _filename: String = match photo_row {
        Some(row) => row.get("filename"),
