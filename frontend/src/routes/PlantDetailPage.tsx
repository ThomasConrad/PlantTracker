...
export const PlantDetailPage: Component = () => {
  ...
  const setupPlantPanelGestures = () => {
    ...
    const clampOverlayTop = (y: number) =>
      Math.max(getFullTop(), Math.min(getMidTop(), y));
    ...
    const onTouchMove = (e: TouchEvent) => {
      ...
      if (
        !dragging &&
        (e.target as HTMLElement)?.closest(".plant-panel-scroll")
      ) {
  ...
  return (
    ...
    >
      {(() => {
        const plant = plantsStore.selectedPlant!;
        const activeTasks = (plant.careTasks || []).filter(
          (t) => !t.archivedAt,
        );
        ...
        if (isMobile()) {
          ...
          return (
            <div class="h-full flex flex-col overflow-hidden relative">
              ...
              <div
                ref={headerRef}
                class="relative z-10 flex items-center justify-between p-4 bg-gradient-to-b from-black/50 to-transparent flex-shrink-0"
                style={{ height: "80px" }}
              >
                <A
                  href="/plants"
                  ...
                  >
                    ...
                    >
                      <path
                        ...
                        stroke-linejoin="round"
                        stroke-width={2}
                        d="M5 8h14M7 8V6a1 1 0 011-1h8a1 1 0 011 1v2m-9 4h8m-8 4h8M6 8v10a2 2 0 002 2h8a2 2 0 002-2V8"
                      />
                    </svg>
                  </button>
                </div>
              ...
              <div
                class="mc-agenda-overlay plant-detail-overlay"
                ref={overlayRef}
              >
                <button
                ...
                >
                  <div class="space-y-5">
                    ...
                    <div class="sticky top-0 z-40 bg-white border-b border-gray-100 shadow-sm px-4 py-3">
                      <div
                        class={`grid gap-3 ${taskStatuses.length > 1 ? "grid-cols-2" : "grid-cols-1"}`}
                      >
                      ...
                      <section class="bg-white shadow-sm rounded-xl border border-gray-200 overflow-hidden">
                        ...
                        <div class="p-4 space-y-3">
                          {taskStatuses.map(({ task, status }) => (
                            <div
                              class={`rounded-xl border p-3 ${status.statusClass}`}
                            >
                              <div class="flex items-start justify-between gap-3">
                                ...
                                <span
                                  class={`inline-flex items-center rounded-full px-2 py-1 text-[11px] font-semibold ${status.badgeClass}`}
                                >
                                  {status.badgeLabel}
                                </span>
                              </div>
                ...
                <div class="fixed inset-0 bg-gray-900/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
                  <div class="bg-white rounded-2xl max-w-md w-full shadow-2xl ring-1 ring-gray-200">
                    <div class="px-6 py-5 border-b border-gray-100">
                      ...
                      <p class="text-sm text-gray-500">
                        {plant.archivedAt
                          ? "This plant will appear in your active list again."
                          : "This plant will be hidden from the default list."}
                      </p>
                    </div>
                    <div class="px-6 py-4">
                    ...
                    <section class="bg-white shadow-sm rounded-xl sm:rounded-2xl border border-gray-200 overflow-hidden">
                      ...
                      <div class="p-4 sm:p-6 space-y-4">
                        {taskStatuses.map(({ task, status, overdue }) => (
                          <div
                            class={`rounded-xl border p-4 ${status.statusClass}`}
                          >
                            <div class="flex items-start justify-between gap-4">
                              <div>
                                <p class="text-sm font-semibold">
                                  {task.icon || "🌱"} {status.title}
                                </p>
                                <p class="text-xs mt-1 opacity-90">
                                  {status.detail}
                                </p>
